...
mkfs.btrfs -L FEDORA ${loop_dev}p2
mount -o compress-force=zstd:9 ${loop_dev}p2 root_partition
tar -C root_partition -xf rootfs.tar

echo "LABEL=FEDORA / btrfs compress-force=zstd:9 0 1" >> root_partition/etc/fstab
